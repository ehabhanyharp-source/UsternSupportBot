...
bot.action('guides', (ctx) => { /* منطق الشروحات الخاص بك كما هو */ });
bot.action('pricing', (ctx) => { /* منطق الأسعار الخاص بك كما هو */ });
bot.action('terms', (ctx) => { /* منطق الشروط الخاص بك كما هو */ });

// ==========================================
// إضافة الدعم البشري (تم دمجها في نظام الـ text الحالي)
// ==========================================
bot.action('human_support', (ctx) => {
    ctx.answerCbQuery();
    activeTickets.set(ctx.from.id, { name: ctx.from.first_name });
    ctx.reply("🎯 اكتب مشكلتك بالتفصيل في رسالة، وسيرد عليك الموظف.");
});
...
bot.on('text', async (ctx) => {
    ...
    // 2. رسالة العميل للبوت
    else if (activeTickets.has(ctx.from.id)) {
        await bot.telegram.sendMessage(SUPPORT_GROUP_ID, `📩 تذكرة من: ${ctx.from.first_name}\n🆔 ID: ${ctx.from.id}\n📝: ${ctx.message.text}\n\n(رد بـ Reply)`);
        ctx.reply("✅ تم إرسال رسالتك لفريق الدعم.");
    }
});

bot.action('back_home', (ctx) => { /* منطق العودة الخاص بك */ });
